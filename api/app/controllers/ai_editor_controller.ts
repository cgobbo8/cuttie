...
/**
 * AI Editor Controller — streams an AI assistant response that can manipulate
 * the video editor via tool calls. The frontend interprets tool calls and
 * applies them to the editor state.
 */
...
- Pour centrer un layer : x = (1080 - width) / 2, y = (1920 - height) / 2.`
    ...
    // Stream fullStream as JSON lines (includes text + tool calls)
    ...
    for await (const part of result.fullStream) {
      if (part.type === 'text-delta') {
        response.response.write(`data: ${JSON.stringify({ type: 'text', text: part.textDelta })}\n\n`)
      } else if (part.type === 'tool-call') {
        response.response.write(`data: ${JSON.stringify({ type: 'tool-call', toolName: part.toolName, args: part.args })}\n\n`)
      } else if (part.type === 'error') {
        response.response.write(`data: ${JSON.stringify({ type: 'error', error: String(part.error) })}\n\n`)
      }
    }
