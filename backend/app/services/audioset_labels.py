"""AudioSet 527 class labels used by PANNs CNN14.

Each index maps to an audio event class from Google's AudioSet ontology.
These labels can be used to configure which audio events the classifier
should listen for when scoring clips.
...
# Reverse lookup: label name -> index
LABEL_TO_INDEX: dict[str, int] = {label: i for i, label in enumerate(AUDIOSET_LABELS)}
