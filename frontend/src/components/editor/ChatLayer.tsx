...
import type { Layer, ChatMessage } from "../../lib/editorTypes";
...
/** Deterministic color from username — same author always gets same color */
...
export default function ChatLayer({ layer, currentTime }: Props) {
  const { chat, transform } = layer;
  if (!chat) return null;

  const { messages, maxVisible, fontSize, fontFamily, showDuration } = chat;

  // Messages visible at currentTime (appeared before now, not yet expired)
