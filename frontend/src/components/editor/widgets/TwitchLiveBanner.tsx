...
const LOOP_PERIOD = 8;       // total cycle
const LOOP_HOLD = 5;         // text visible, idle
const LOOP_SWEEP = 0.5;      // curtain passes across
...
/** Ease-in-out with a punch: fast start, brief slow-down mid, fast exit */
function easeSweep(t: number): number {
  const c = Math.min(1, Math.max(0, t));
  // Custom cubic: accelerate, slight brake, accelerate out
  if (c < 0.4) return 0.5 * easeOutPower3(c / 0.4);
  if (c < 0.6) return 0.5 + 0.12 * ((c - 0.4) / 0.2); // slow crawl (the "cover" moment)
...
/** Small bounce for loop badge kick: 1.0 → 1.08 → 1.0 */
...
export default function TwitchLiveBanner({ props, width, height, currentTime }: Props) {
  ...
  // ═══════════════════════════════════════════════════════════
  // ENTRANCE (first ~0.65s)
  // ═══════════════════════════════════════════════════════════

  const entranceDone = time > BADGE_END;

  // Purple curtain entrance (in then out)
  const eCurtainIn = easeOutPower3(progress(time, CURTAIN_IN_START, CURTAIN_IN_END));
  const eCurtainOut = easeOutPower3(progress(time, CURTAIN_OUT_START, CURTAIN_OUT_END));
  const eContentIn = easeOutPower3(progress(time, CONTENT_IN_START, CONTENT_IN_END));
  const eBadgeP = springPop(progress(time, BADGE_START, BADGE_END));
  const eBadgeScale = time < BADGE_START ? 0 : eBadgeP;

  // ═══════════════════════════════════════════════════════════
  // LOOP (after entrance, every LOOP_PERIOD seconds)
  // ═══════════════════════════════════════════════════════════

  const loopTime = entranceDone
    ? ((time - BADGE_END) % LOOP_PERIOD + LOOP_PERIOD) % LOOP_PERIOD
    : -1;
  ...
  // Entrance curtain (only during entrance)
    ...
    >
      ...
      <div
        style={{
          ...
          filter: entranceDone
            ? `drop-shadow(0 0 ${Math.round(8 + 6 * glowSin)}px rgba(145,70,255,${0.2 + 0.15 * glowSin}))`
            : undefined,
          zIndex: 2,
          position: "relative",
        }}
      >
        ...
        <div
          style={{
            height: badgeH,
            background: "linear-gradient(180deg, #A970FF 0%, #8B3FE8 100%)",
            display: "flex",
            alignItems: "center",
            gap: Math.round(badgeH * 0.2),
            padding: `0 ${Math.round(badgeH * 0.4)}px`,
          }}
        >
          <div
            style={{
              width: Math.max(6, Math.round(badgeH * 0.18)),
              height: Math.max(6, Math.round(badgeH * 0.18)),
              borderRadius: "50%",
              background: "#FF4444",
              boxShadow: `0 0 ${Math.round(6 + 4 * livePulse)}px rgba(255,68,68,${livePulse})`,
              opacity: livePulse,
              flexShrink: 0,
            }}
          />
      ...
      >
        ...
        >
          ...
          <div
            style={{
              background: "linear-gradient(135deg, rgba(20,20,25,0.93) 0%, rgba(10,10,14,0.96) 100%)",
              backdropFilter: "blur(12px)",
              WebkitBackdropFilter: "blur(12px)",
              display: "flex",
              alignItems: "center",
              gap: Math.round(urlH * 0.1),
              padding: `0 ${Math.round(urlH * 0.2)}px`,
            }}
          >
            {hasLogo && (
              <img
        ...
        {entranceCurtainVisible && (
          <div
            style={{
              ...
              top: 0,
              bottom: 0,
              left: `${entranceCurtainLeft}%`,
              right: `${entranceCurtainRight}%`,
              background: "linear-gradient(135deg, #A970FF 0%, #8B3FE8 50%, #7B2FD8 100%)",
              zIndex: 1,
            }}
          />
        )}
        ...
        {inSweep && (
          <div
            style={{
              ...
              bottom: 0,
              width: "100%",
              transform: `translateX(${loopCurtainX}%)`,
              background: "linear-gradient(135deg, #A970FF 0%, #8B3FE8 50%, #7B2FD8 100%)",
              zIndex: 1,
            }}
          />
